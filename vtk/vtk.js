...
import vtkPiecewiseWidget from './PiecewiseWidget/index.js';
...
const lookupTableProxy = vtkLookupTableProxy.newInstance({ lookupTable: lookupTable });
const piecewiseFunctionProxy = vtkPiecewiseFunctionProxy.newInstance({ piecewiseFunction: piecewiseFunction });
...
function changePreset(delta = 0) {
    ...
    let presetName = local_rgbPresetNames[presetIndex]
    let presetObj = local_getPresetByName(presetName)
    lookupTable.applyColorMap(presetObj);

    const ctRange = getColorFunctionRangeFromPreset(presetName);
    let opRange = getOpacityRangeFromPreset(presetName);

    console.log("数据范围:", globalDataRange)
    console.log("传输曲线范围:", ctRange)
    console.log("透明度曲线范围:", opRange)
    lookupTable.setMappingRange(...ctRange);
    lookupTable.updateRange();

    let opPoints = resetOpacityFunction(piecewiseFunctionProxy, opRange, presetName)
...
console.log(local_rgbPresetNames)
...
function dicomParser_loadSingleData(index, max) {
    ...
    axios.get(url, { responseType: "arraybuffer" }).then((resp) => {
        ...
        var pixelValueArr = new Int16Array(pixelData.length)
        for (let i = 0; i < pixelData.length; i++) {
            pixelValueArr[i] = pixelData[i] * slope + intercept
        }
        dataArr = null
        dataSet = null
        imageList[index - 1] = { width: dataWidth, height: dataHeight, data: pixelValueArr }
        // pixelData = null
...
function initImageDataForVTK() {
    ...
    widget.setOpacityPoints(presetPara.opPoints);
    ...
    mapper.setSampleDistance(0.9765625)
    ...
    property.setScalarOpacityUnitDistance(0, 1.4181691598216133);
    property.setUseGradientOpacity(0, true);
    property.setGradientOpacityMinimumValue(0, 0);
    property.setGradientOpacityMaximumValue(0, (globalDataRange[1] - globalDataRange[0]) * 0.01);
...
function getShiftedOpacityFromPreset(
    presetName,
    effectiveRange,
    shift
) {
